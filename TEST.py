...
#print(drivers_json["D1"])
#print(passengers_json)
...
def get_loc(df):
    ...
    for i, j in A:
        if i in D and j in NP:
            ...
            print((stedsnavn1, stedsnavn2))
            distance = distance_matrix[(stedsnavn1, stedsnavn2)]
            t_ij[(i,j)] = distance
        print("Hergrg")
        if i in D and j == (i + nr_drivers + (2*nr_passengers)):
            stedsnavn1 = drivers_json[i]["Origin location"]
            stedsnavn2 = drivers_json[i]["Destination location"]
            distance = distance_matrix[(stedsnavn1, stedsnavn2)]
            t_ij[(i,j)] = distance
        if i in NP and j in NP:
            stedsnavn1 = passengers_json[i]["Origin location"]
            stedsnavn2 = passengers_json[j]["Origin location"]
            distance = distance_matrix[(stedsnavn1, stedsnavn2)]
            t_ij[(i,j)] = distance
        if i in NP and j in ND:
            stedsnavn1 = passengers_json[i]["Origin location"]
            stedsnavn2 = passengers_json[j - nr_passengers]["Destination location"]
            distance = distance_matrix[(stedsnavn1, stedsnavn2)]
            t_ij[(i,j)] = distance
        if i in ND and j in ND:
            stedsnavn1 = passengers_json[i - nr_passengers]["Destination location"]
            stedsnavn2 = passengers_json[j - nr_passengers]["Destination location"]
            distance = distance_matrix[(stedsnavn1, stedsnavn2)]
            t_ij[(i,j)] = distance
